...
def create_initial_data(sender, **kwargs):
    if sender.name == 'core':
        ...
        Dish.objects.get_or_create(name="Олів'є по-домашньому", category=salads, price=89)
        Dish.objects.get_or_create(name="Грецький салат з бринзою", category=salads, price=115)
        Dish.objects.get_or_create(name="Салат з куркою та ананасом", category=salads, price=100)

        Dish.objects.get_or_create(name="Fanta (1,25 л)", category=drinks, price=40)
        Dish.objects.get_or_create(name="Сoca cola (750 мл)", category=drinks, price=29)
        Dish.objects.get_or_create(name="Pepsi (1 л)", category=drinks, price=35)

        Dish.objects.get_or_create(name="Борщ український зі сметаною", category=first_courses, price=90)
        Dish.objects.get_or_create(name="Курячий бульйон з локшиною", category=first_courses, price=75)
        Dish.objects.get_or_create(name="Грибна юшка з перловкою", category=first_courses, price=87)

        Dish.objects.get_or_create(name="Картопля по-селянськи з грибами", category=second_courses, price=103)
        Dish.objects.get_or_create(name="Тушкована капуста з ковбасками", category=second_courses, price=105)
        Dish.objects.get_or_create(name="Каша гречана з печінкою", category=second_courses, price=100)

        Dish.objects.get_or_create(name="Котлета по-київськи з картопляним пюре", category=main_courses, price=130)
        Dish.objects.get_or_create(name="Стейк зі свинини з овочами гриль", category=main_courses, price=179)
        Dish.objects.get_or_create(name="Філе куряче в вершковому соусі з рисом", category=main_courses, price=145)

        Dish.objects.get_or_create(name="Сирники зі сметаною та варенням", category=desserts, price=79)
        Dish.objects.get_or_create(name="Штрудель яблучний з морозивом", category=desserts, price=95)
        Dish.objects.get_or_create(name="Медівник домашній", category=desserts, price=70)
